...
$loop_setting = wc_get_loop_prop( 'goso_woo_settings' );
...
if ( goso_is_shop_archive() || goso_is_shop_catelog() ) {
	$data_class = get_theme_mod( 'goso_shop_product_pagination', 'pagination' ) . '-pagination';
}
...
if ( 'loadmore' == $pagination && ! empty( $loop_setting ) && $shortcode ) {
	...
	$url_query  = goso_woo_ajax_url( $loop_setting );
	$nonce      = wp_create_nonce( 'ajax-nonce' );
	$base       = esc_url_raw( admin_url( 'admin-ajax.php?action=goso_ajax_load_products&requestid=' . $nonce . '&' . $url_query . '&product-page=%#%' ) );
}
