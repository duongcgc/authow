...
$search_args = array(
	's'              => $args['search_query'],
	'posts_per_page' => get_theme_mod( 'goso_woocommerce_search_included_total', 5 ),
	'post_type'      => 'post',
);

$product_search_post_query = new WP_Query( $search_args );
if ( $product_search_post_query->have_posts() ):?>
    <div class="goso-border-arrow goso-homepage-title goso-magazine-title style-7 pcalign-left pciconp-right pcicon-right">
        <h3 class="inner-arrow">
            <a href="<?php echo esc_url( get_search_link( $args['search_query'] ) ); ?>">
				<?php echo goso_woo_translate_text( 'goso_woo_trans_blog_search_result' ); ?>
            </a>
        </h3>
    </div>
	<?php
	...
	?>
    <div class="pcbg-paging goso-pagination pcbg-paging-align-center goso-ajax-more">
        <a class="goso-ajax-more-button" href="<?php echo esc_url( get_search_link( $args['search_query'] ) ); ?>">
            <span class="ajax-more-text"><?php echo goso_woo_translate_text( 'goso_woo_trans_showmore_result' ); ?></span>
        </a>
    </div>
<?php endif;
