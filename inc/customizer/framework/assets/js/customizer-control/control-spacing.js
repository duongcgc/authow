wp.customize.controlConstructor['authow-fw-spacing'] = wp.customize.controlConstructor.default.extend({
	...
	ready: function() {
		...
		_.each( ['top', 'bottom', 'left', 'right'], function( dimension, index ) {
			...
			if ( control.container.has( '.' + dimension ).size() ) {
				...
				// if ( false === authowValidateCSSValue( control.setting._value[ dimension ] ) ) {
				...
				control.container.on( 'change keyup paste', '.' + dimension + ' input', function() {
					...
					// Validate the value and show a warning if it's invalid.
					if ( false === authowValidateCSSValue( subValue ) ) {
