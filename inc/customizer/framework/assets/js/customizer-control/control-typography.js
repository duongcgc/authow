wp.customize.controlConstructor['authow-fw-typography'] = wp.customize.controlConstructor.default.extend({
	...
	ready: function() {
		...
		renderSubControl = function( fontFamily, sub, startValue ) {
			...
			_.each( authowAllFonts, function( font, i ) {
			...
			if ( true === isStandard ) {
				...
				if ( 2 > subList.length ) {
					...
					control.container.find( '.authow-fw-' + sub + '-wrapper' ).css( 'display', 'none' );
				} else {
					control.container.find( '.authow-fw-' + sub + '-wrapper' ).css( 'display', 'block' );
				}
		...
		picker = this.container.find( '.authow-fw-color-control' );
