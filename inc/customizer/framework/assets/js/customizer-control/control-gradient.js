wp.customize.controlConstructor['authow-fw-gradient'] = wp.customize.controlConstructor.default.extend({
    ...
    ready: function() {
        ...
        renderLocation = function($range, location) {
            var thisInput,
                inputDefault,
                range_value,
                $reset = $range.closest( '.range' ).find('.authow-fw-slider-reset');

            $range.on( 'mousedown', function() {
                jQuery( this ).mousemove( function() {
                    range_value = jQuery(this).val();
                    jQuery( this ).closest( '.range' ).find( '.authow_fw_range_value .value' ).text( range_value );
            ...
            $reset.on( 'click', function() {
                ...
                thisInput.change();
                jQuery( this ).closest( '.range' ).find( '.authow_fw_range_value .value' ).text( inputDefault );
            });
        };

        // Change color
        beginpicker = this.container.find( '.authow-fw-begin-color' ).wpColorPicker({
            change: function() {
                setTimeout( function() {
        ...
        endpicker = this.container.find( '.authow-fw-end-color' ).wpColorPicker({
            change: function() {
                setTimeout( function() {
        ...
        // Range
        renderLocation(this.container.find( '.authow-fw-range-begin input[type=range]' ), 'beginlocation', value);
        renderLocation(this.container.find( '.authow-fw-range-end input[type=range]' ), 'endlocation', value);
        renderLocation(this.container.find( '.authow-fw-range-degree input[type=range]' ), 'degree', value);
    },
