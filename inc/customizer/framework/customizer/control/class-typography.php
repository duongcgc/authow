...
class Typography extends Control_Abstract {
	 ...
	 * @var string
	 */
	public $type = 'authow-fw-typography';
		...
		<div class="wrapper">
			...
			<# if ( false !== data.default['font-family'] ) { #>
				...
				<div class="font-family">
					<h5>Font Family</h5>
					<select id="authow-fw-typography-font-family-{{{ data.id }}}" placeholder="<?php echo esc_attr__('Select Font Family', 'authow'); ?>"></select>
				</div>
				<# if ( true === data.show_variants || false !== data.default.variant ) { #>
					<div class="variant hide-on-standard-fonts authow-fw-variant-wrapper">
						<h5>Font Variant</h5>
						<select class="variant" id="authow-fw-typography-variant-{{{ data.id }}}"></select>
					</div>
				<# } #>
				<# if ( true === data.show_subsets ) { #>
					<div class="subsets hide-on-standard-fonts authow-fw-subsets-wrapper">
						<h5>Font Subsets</h5>
						<select class="subset" id="authow-fw-typography-subsets-{{{ data.id }}}"></select>
					</div>
				<# } #>
			<# } #>
			...
			<# if ( false !== data.default['text-transform'] ) { #>
				<div class="text-transform">
					<h5>Transform</h5>
					<select id="authow-fw-typography-text-transform-{{{ data.id }}}">
			...
			<# if ( false !== data.default['color'] ) { #>
				<div class="color">
					<h5>Color</h5>
					<input type="text" data-palette="{{ data.palette }}" data-default-color="{{ data.default['color'] }}" value="{{ data.value['color'] }}" class="authow-fw-color-control color-picker"/>
				</div>
			<# } #>
		</div>
		<?php
