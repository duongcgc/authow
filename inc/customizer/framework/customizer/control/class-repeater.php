...
class Repeater extends Control_Abstract {
	 ...
	 * @var string
	 */
	public $type = 'authow_fw_repeater';
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		if ( empty( $this->button_label ) ) {
			$this->button_label = esc_attr__( 'Add new', 'authow' ) . ' ' . $this->row_label['value'];
		}
			...
			if ( isset( $value['type'] ) && ( 'dropdown-pages' === $value['type'] ) ) {
				...
				$dropdown = wp_dropdown_pages(
					array(
						'name'              => '',
						'echo'              => 0,
						'show_option_none'  => esc_attr__( 'Select a Page', 'authow' ),
						'option_none_value' => '0',
						'selected'          => '',
					)
				);
	...
	protected function render_content() {
		...
		<?php if ( isset( $this->choices['limit'] ) ) : ?>
			<p class="limit"><?php printf( esc_attr__( 'Limit: %s rows', 'authow' ), esc_html( $this->choices['limit'] ) ); ?></p>
		<?php endif; ?>
		...
		<script type="text/html" class="customize-control-repeater-content">
			...
			<li class="repeater-row minimized" data-row="{{{ index }}}">
				...
				<div class="repeater-row-content">
					<# _.each( data, function( field, i ) { #>
						...
						<div class="repeater-field repeater-field-{{{ field.type }}}">
							...
							<# } else if ( 'color' === field.type ) { #>
								...
								<label>
									...
									<# if ( field.description ) { #>
										<span class="description customize-control-description">{{{ field.description }}}</span>
									<# } #>
									<input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php echo esc_attr__( 'Hex Value', 'authow' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
							...
							<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
								...
								<figure class="authow-fw-image-attachment" data-placeholder="<?php echo esc_attr__( 'No Image Selected', 'authow' ); ?>" >
									<# if ( field.default ) { #>
										<# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
										<img src="{{{ defaultImageURL }}}">
									<# } else { #>
										<?php echo esc_attr__( 'No Image Selected', 'authow' ); ?>
									<# } #>
								</figure>

								<div class="actions">
									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php echo esc_attr__( 'Remove', 'authow' ); ?></button>
									<button type="button" class="button upload-button" data-label=" <?php echo esc_attr__( 'Add Image', 'authow' ); ?>" data-alt-label="<?php echo esc_attr__( 'Change Image', 'authow' ); ?>" >
										<# if ( field.default ) { #>
											<?php echo esc_attr__( 'Change Image', 'authow' ); ?>
										<# } else { #>
											<?php echo esc_attr__( 'Add Image', 'authow' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
							...
							<# } else if ( field.type === 'upload' ) { #>
								...
								<figure class="authow-fw-file-attachment" data-placeholder="<?php echo esc_attr__( 'No File Selected', 'authow' ); ?>" >
									<# if ( field.default ) { #>
									...
									<# } else { #>
										<?php echo esc_attr__( 'No File Selected', 'authow' ); ?>
									<# } #>
								</figure>

								<div class="actions">
									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php echo esc_attr__( 'Remove', 'authow' ); ?></button>
									<button type="button" class="button upload-button" data-label="<?php echo esc_attr__( 'Add File', 'authow' ); ?>" data-alt-label="<?php echo esc_attr__( 'Change File', 'authow' ); ?>" >
										<# if ( field.default ) { #>
											<?php echo esc_attr__( 'Change File', 'authow' ); ?>
										<# } else { #>
											<?php echo esc_attr__( 'Add File', 'authow' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
										<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
					...
					<# }); #>
					<button type="button" class="button-link repeater-row-remove"><?php echo esc_attr__( 'delete', 'authow' ); ?></button>
				</div>
			</li>
		</script>
		<?php
