...
class Color extends Control_Abstract {
	...
	protected function content_template() {
		...
		<label>
			...
			<# } #>
			<div>
				<input type="text" data-palette="{{ data.palette }}" data-default-color="{{ data.default }}" data-alpha="{{ data.choices['alpha'] }}" value="{{ data.value }}" class="authow-fw-color-control" {{{ data.link }}} />
			</div>
		</label>
		<?php
	}
