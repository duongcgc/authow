...
if ( ! class_exists( 'Goso_Pinterest' ) ):

	class Goso_Pinterest {
				...
				if ( ! is_wp_error( $response ) ) {
					...
					if ( ! empty( $matches[1] ) && count( $matches[1] ) ) {
						...
						if ( ! isset( $pinterest_json['resourceDataCache'][1]['data']['board_feed'] ) ) {
							$output['error'] = esc_html__( 'The pinterest data is not set, please check the ID', 'authow' );
						} elseif ( isset( $pinterest_json["resourceDataCache"][0]['data']['type'] ) && $pinterest_json["resourceDataCache"][0]['data']['type'] !== 'board' ) {
							$output['error'] = esc_html__( 'Invalid pinterest data for  <strong>' . $username . '</strong> please check the <em>user/board_id</em>', 'authow' );
						} else {
...
if( ! class_exists( 'Goso_Pinterest_Widget' ) ) {
	class Goso_Pinterest_Widget extends WP_Widget {
		...
		function form( $instance ) {
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'numbers' ) ); ?>"><?php esc_html_e( 'Numbers image to show:', 'authow' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'cache' ) ); ?>"><?php esc_html_e( 'Cache life time ( unit is seconds ):', 'authow' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'follow' ) ); ?>"><?php esc_html_e( 'Display more link with username text?:', 'authow' ); ?></label>
