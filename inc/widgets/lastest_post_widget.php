...
add_action( 'widgets_init', 'goso_latest_news_load_widget' );

function goso_latest_news_load_widget() {
	register_widget( 'goso_latest_news_widget' );
}

if ( ! class_exists( 'goso_latest_news_widget' ) ) {
	class goso_latest_news_widget extends WP_Widget {
		...
		function __construct() {
			/* Widget settings. */
			$widget_ops = array(
				'classname'   => 'goso_latest_news_widget',
				'description' => esc_html__( 'A widget that displays your recent posts from all categories or a category', 'authow' )
			);

			/* Widget control settings. */
			$control_ops = array( 'id_base' => 'goso_latest_news_widget' );

			/* Create the widget. */
			global $wp_version;
			if ( 4.3 > $wp_version ) {
				$this->WP_Widget( 'goso_latest_news_widget', goso_get_theme_name('.Authow',true).esc_html__( 'Recent Posts', 'authow' ), $widget_ops, $control_ops );
			} else {
				parent::__construct( 'goso_latest_news_widget', goso_get_theme_name('.Authow',true).esc_html__( 'Recent Posts', 'authow' ), $widget_ops, $control_ops );
			}
		}
		...
		function widget( $args, $instance ) {
			...
			if ( $loop->have_posts() ) :
				...
				?>
                <ul id="goso-latestwg-<?php echo sanitize_text_field( $rand ); ?>" class="side-newsfeed<?php if ( $twocolumn && ! $allfeatured ): echo ' goso-feed-2columns'; if ( $featured ) { echo ' goso-2columns-featured'; } else { echo ' goso-2columns-feed'; } endif; if ( $showborder ) { echo ' goso-rcpw-hborders'; } if ( $dotstyle ) { echo ' pctlst pctl-' . $dotstyle; } ?>">
					<?php $num = 1;
				...
				$attrstyle = '';
				if ( $ptfsfe ) {
					$attrstyle .= '.widget ul#goso-latestwg-' . $rand . ' li.featured-news .side-item .side-item-text h4 a{ font-size: ' . $ptfsfe . 'px; }';
				}
				if ( $ptfs ) {
					$attrstyle .= '.widget ul#goso-latestwg-' . $rand . ' li:not(.featured-news) .side-item .side-item-text h4 a{ font-size: ' . $ptfs . 'px; }';
				}
				if ( $pmfs ) {
					$attrstyle .= '.widget ul#goso-latestwg-' . $rand . ' li .side-item .side-item-text .side-item-meta{ font-size: ' . $pmfs . 'px; }';
				}
				if ( $imgwidth ) {
					$attrstyle .= '.widget ul#goso-latestwg-' . $rand . ' li .goso-image-holder.small-fix-size{ width: ' . $imgwidth . 'px; }';
				}
				if ( $row_gap ) {
					$attrstyle .= '.widget ul#goso-latestwg-' . $rand . '.side-newsfeed:not(.goso-feed-2columns) li{ margin-bottom: ' . $row_gap . 'px; padding-bottom: ' . $row_gap . 'px; }';
					$attrstyle .= '.widget ul#goso-latestwg-' . $rand . '.goso-feed-2columns li{ margin-bottom: ' . $row_gap . 'px; }';
				}
				if ( $image_type == 'horizontal' ) {
					$attrstyle .= '#goso-latestwg-' . $rand . ' .goso-image-holder:before{ padding-top: 66.6667%; }';
				} elseif ( $image_type == 'square' ) {
					$attrstyle .= '#goso-latestwg-' . $rand . ' .goso-image-holder:before{ padding-top: 100%; }';
				} elseif ( $image_type == 'vertical' ) {
					$attrstyle .= '#goso-latestwg-' . $rand . ' .goso-image-holder:before{ padding-top: 135.4%; }';
				}
