...
if ( ! class_exists( 'Goso_Product_Filter' ) ) {
	class Goso_Product_Filter extends WPH_Widget {
		...
		function __construct() {
			...
			$categories_array = array(
				esc_html__( 'All categories', 'authow' ) => 'all',
			);
			...
			$args['fields'] = array(
				array(
					'id'   => 'title',
					'type' => 'text',
					'std'  => esc_html__( 'Filter by', 'authow' ),
				...
				array(
					...
					'type'   => 'dropdown',
					'std'    => '',
					'name'   => esc_html__( 'Attribute', 'authow' ),
				...
				/*array(
					...
					'type'    => 'select2',
					'default' => array( 'all' ),
					'name'    => esc_html__( 'Show on category', 'authow' ),
				...
				array(
					...
					'type'   => 'dropdown',
					'std'    => 'and',
					'name'   => esc_html__( 'Query type', 'authow' ),
					'fields' => array(
				...
				array(
					...
					'type'   => 'dropdown',
					'std'    => 'list',
					'name'   => esc_html__( 'Display type', 'authow' ),
					'fields' => array(
						esc_html__( 'list', 'authow' )      => 'list',
						esc_html__( '2 columns', 'authow' ) => 'double',
						esc_html__( 'inline', 'authow' )    => 'inline',
						esc_html__( 'Dropdown', 'authow' )  => 'dropdown'
					)
				),
				array(
					'id'          => 'size',
					'type'        => 'dropdown',
					'std'         => 'normal',
					'name'        => esc_html__( 'Swatches size', 'authow' ),
					'description' => esc_html__( 'Only work with Color & Image Attribute Type', 'authow' ),
					'fields'      => array(
						esc_html__( 'normal', 'authow' ) => 'normal',
						esc_html__( 'large', 'authow' )  => 'large',
						esc_html__( 'small', 'authow' )  => 'small',
					)
				),
				array(
					'id'     => 'labels',
					'type'   => 'dropdown',
					'std'    => 'on',
					'name'   => esc_html__( 'Show labels', 'authow' ),
					'fields' => array(
						esc_html__( 'ON', 'authow' )  => 'on',
						esc_html__( 'OFF', 'authow' ) => 'off'
					)
				),
				array(
					'id'     => 'count',
					'type'   => 'dropdown',
					'std'    => 'on',
					'name'   => esc_html__( 'Show product count number', 'authow' ),
					'fields' => array(
						esc_html__( 'ON', 'authow' )  => 'on',
						esc_html__( 'OFF', 'authow' ) => 'off'
					)
				...
				array(
					...
					'type'   => 'dropdown',
					'std'    => 'on',
					'name'   => esc_html__( 'Show tooltips', 'authow' ),
					'fields' => array(
						esc_html__( 'OFF', 'authow' ) => 'off',
						esc_html__( 'ON', 'authow' )  => 'on',
					)
		...
		protected function layered_nav_dropdown( $terms, $taxonomy, $query_type ) {
			...
			if ( $taxonomy !== $this->get_current_taxonomy() ) {
				...
				$any_label      = apply_filters( 'woocommerce_layered_nav_any_label', sprintf( __( 'Any %s', 'authow' ), $taxonomy_label ), $taxonomy_label, $taxonomy );
		...
		protected function layered_nav_checkbox_list( $terms, $taxonomy, $query_type, $instance ) {
			$query_type  = isset( $instance['query_type'] ) ? $instance['query_type'] : 'and';
			$title       = isset( $instance['filter-title'] ) ? $instance['filter-title'] : esc_html__( 'Filter by', 'authow' );
	...
	add_action( 'widgets_init', function () {
		register_widget( 'Goso_Product_Filter' );
	} );
