...
if ( ! class_exists( 'Goso_Login_Register_Widget' ) ) {
	class Goso_Login_Register_Widget extends WP_Widget {
		...
		function __construct() {
			...
			global $wp_version;
			if ( 4.3 > $wp_version ) {
				$this->WP_Widget( 'penci_login_register_widget', penci_get_theme_name('.Authow',true).esc_html__( 'Login/Register', 'authow' ), $widget_ops, $control_ops );
			} else {
				parent::__construct( 'penci_login_register_widget', penci_get_theme_name('.Authow',true).esc_html__( 'Login/Register', 'authow' ), $widget_ops, $control_ops );
			}
		}
