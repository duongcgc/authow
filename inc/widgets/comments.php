...
if ( ! class_exists( 'goso_comments_widget' ) ) {
	class goso_comments_widget extends WP_Widget {
		...
		public function authow_widget_defaults() {
			return array(
				'title'       => esc_html__( 'Recent Comments', 'authow' ),
				'number'      => 5,
				'avatar_size' => 70,
				'length'      => 12,
			);
		...
		function form( $instance ) {
            ...
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php esc_html_e( 'Number of comments to show:', 'authow' ) ?></label>
            ...
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'length' ) ); ?>"><?php esc_html_e( 'Custom Comment Content Words Length:', 'authow' ) ?></label>
