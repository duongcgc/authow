...
class Goso_Authow_Dashboard {
	...
	public function add_menu() {
		...
		add_submenu_page( 'authow_dashboard_welcome', esc_html__( 'Custom fonts', 'authow' ), esc_html__( 'Custom Fonts', 'authow' ), 'manage_options', 'authow_custom_fonts', array(
			$this,
			'custom_fonts'
		) );
	...
	public function get_wel_page_title() {
		...
		return $wel_page_title && get_theme_mod( 'activate_white_label' ) ? $wel_page_title : 'Authow';
	}

	public function replace_text_submenu() {
		global $submenu;
		$submenu['authow_dashboard_welcome'][0][0] = esc_html__( 'Welcome', 'authow' );
	}

	public function update() {
		if ( ! empty( $_POST ) && isset( $_POST['_page'] ) && $_POST['_page'] === 'authow_custom_fonts' ) {
			$data = $_POST;

			$fonts = array();

			foreach ( $_POST as $key => $value ) {
				if ( strpos( $key, 'authow_' ) !== false ) {
					$fonts[ $key ] = $value;
				}
			}
	...
	public function add_bar_menu() {
		...
		$wp_admin_bar->add_menu( array(
			...
			'href'   => admin_url( 'admin.php?page=authow_dashboard_welcome' )
		) );
	}
	...
	public function enqueue_scripts( $hook ) {
		...
		wp_enqueue_script( "authow-button-script", get_template_directory_uri() . '/inc/woocommerce/js/jquery-grid-picker.js?v=8.0.7', array( 'jquery' ) );
		wp_enqueue_script( "authow-dashboard-script", get_template_directory_uri() . '/inc/dashboard/js/script.js?v=8.0.7', array( 'jquery' ) );
	...
	public function redirect() {
		...
		if ( is_admin() && isset( $_GET['activated'] ) && 'themes.php' === $pagenow ) {
			wp_safe_redirect( admin_url( "admin.php?page=authow_dashboard_welcome" ) );
			exit;
		}
	}
}
