...
class penci_product_swatches {
	...
	public function __construct() {
		$this->types = array(
			'color' => esc_html__( 'Color', 'authow' ),
			'image' => esc_html__( 'Image', 'authow' ),
		...
		wp_localize_script(
			...
			array(
				'i18n'        => array(
					...
					'mediaButton' => esc_html__( 'Use image', 'authow' ),
				),
	...
	public function product_option_terms( $taxonomy, $index ) {
        ...
        <select multiple="multiple" data-placeholder="<?php esc_attr_e( 'Select terms', 'authow' ); ?>"
	...
	public function add_new_attribute_ajax() {
		...
		if ( empty( $name ) || empty( $swatch ) || empty( $tax ) || empty( $type ) ) {
			wp_send_json_error( esc_html__( 'Not enough data', 'authow' ) );
		}

		if ( ! taxonomy_exists( $tax ) ) {
			wp_send_json_error( esc_html__( 'Taxonomy is not exists', 'authow' ) );
		}

		if ( term_exists( $_POST['name'], $_POST['tax'] ) ) {
			wp_send_json_error( esc_html__( 'This term is exists', 'authow' ) );
		}
		...
		wp_send_json_success(
			array(
				'msg'  => esc_html__( 'Added successfully', 'authow' ),
				'id'   => $term->term_id,
				'slug' => $term->slug,
				'name' => $term->name,
			)
