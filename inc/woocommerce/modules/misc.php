...
class goso_product_misc {

	protected static $instance;

	function __construct() {

		add_action( 'wp_ajax_goso_update_cart_item', array( $this, 'goso_update_cart_item' ) );
		add_action( 'wp_ajax_nopriv_goso_update_cart_item', array( $this, 'goso_update_cart_item' ) );

		add_action( 'wp_ajax_goso_ajax_load_products', array( $this, 'ajax_load_products' ) );
		add_action( 'wp_ajax_nopriv_goso_ajax_load_products', array( $this, 'ajax_load_products' ) );

		add_action( 'wp_ajax_goso_get_product_info', array( $this, 'get_product_info' ) );
		add_action( 'wp_ajax_nopriv_goso_get_product_info', array( $this, 'get_product_info' ) );

		add_filter( 'woocommerce_sale_flash', array( $this, 'product_label' ), 10 );
		add_action( 'woocommerce_before_single_product_summary', array( $this, 'goso_top_relate_products' ), 5 );

		add_action( 'goso_woocommerce_before_main_shop_content_right', function () {
			echo '<div class="goso-wofilter-inner">';
		}, 4 );
		add_action( 'goso_woocommerce_before_main_shop_content_right', array( $this, 'product_grid_label' ), 5 );
		add_action( 'goso_woocommerce_before_main_shop_content_right', array( $this, 'product_style_select' ), 5 );
		add_action( 'goso_woocommerce_before_main_shop_content_right', array(
			$this,
		...
		if ( ! get_theme_mod( 'goso_woo_cat_enable_sidebar' ) ) {
		...
		add_action( 'goso_footer_nav_cart_item', array( $this, 'mobile_cart_totals' ) );
		add_action( 'woocommerce_add_to_cart_fragments', array( $this, 'mobile_cart_totals_fragment' ) );

		add_action( 'wp_ajax_goso_add_to_cart_variable', array(
			$this,
			'goso_add_to_cart_variable'
		) );
		add_action( 'wp_ajax_nopriv_goso_add_to_cart_variable', array(
			$this,
			'goso_add_to_cart_variable'
		) );

		add_action( 'woocommerce_after_shop_loop', array( $this, 'blog_search_result' ), 100 );
		add_action( 'goso_after_search_loop', array( $this, 'product_search_result' ), 100 );
	}
	...
	public function goso_top_relate_products() {
		if ( wc_get_loop_prop( 'quickview', false ) || ! get_theme_mod( 'goso_single_product_top_related_product', true ) ) {
			return;
		}
		$current_id          = apply_filters( 'goso_top_relate_products_exclude', get_the_ID() );
		...
		if ( $top_relate_products ) {
			echo '<div class="goso-top-relate-post"><ul>';
			$count = 0;
			foreach ( $top_relate_products as $product_item ) {
				...
				echo '<li class="top-ralate-item ' . $classes . '"><a href="' . get_permalink( $id ) . '"><span class="item-icon"><i class="gosoicon-' . $classes . '-chevron"></i></span></a>';
	...
	public function goso_product_quantity( $product ) {
	...
	public function goso_update_cart_item() {
	...
	function product_label() {
		...
		$percentage_label = get_theme_mod( 'goso_woo_label_percentage', true );
		...
		if ( $product->is_on_sale() ) {
			...
			if ( $percentage ) {
				...
				$output[] = '<span class="onsale product-label">' . goso_woo_translate_text( 'goso_woo_trans_sale' ) . '</span>';
			}
		}

		if ( ! $product->is_in_stock() ) {
			$output[] = '<span class="out-of-stock product-label">' . goso_woo_translate_text( 'goso_woo_trans_sold_out' ) . '</span>';
		}

		if ( $product->is_featured() && get_theme_mod( 'goso_woo_label_hot_product', true ) ) {
			$output[] = '<span class="featured product-label">' . goso_woo_translate_text( 'goso_woo_trans_hot' ) . '</span>';
		}

		if ( $this->is_new_label_needed( $product->get_id() ) ) {
			$output[] = '<span class="new product-label">' . goso_woo_translate_text( 'goso_woo_trans_new' ) . '</span>';
		}

		if ( $output ) {
			echo '<div class="product-labels labels-' . get_theme_mod( 'goso_woo_label_style', 'square' ) . '">' . implode( '', $output ) . '</div>';
		}
	}

	public function is_new_label_needed( $product_id ) {
		$enable       = get_theme_mod( 'goso_woo_label_new_product', true );
		$new          = goso_get_single_product_meta( $product_id, 'product_extra_options', 'permanent_new_label' );
		$newness_days = get_theme_mod( 'goso_woo_label_new_product_period', 7 );
	...
	public function product_grid_label() {
		...
		$default_posts_per_page    = array( goso_woo_get_products_per_page() );
		$per_page_options          = get_theme_mod( 'goso_woo_post_per_page_variations', '9,24,36,-1' );
        ...
        <div class="goso-products-per-page">
			<span class="per-page-title">
				<?php echo goso_woo_translate_text( 'goso_woo_trans_show' ); ?>
			</span>
			...
			foreach ( $products_per_page_options as $key => $value ) :
				...
				$link = add_query_arg( $args, goso_shop_page_link( true ) );

				if ( (int) goso_woo_get_products_per_page() === (int) $value ) { // phpcs:ignore
                ...
                <a rel="nofollow noopener" href="<?php echo esc_url( $link ); ?>"
                   class="per-page-variation<?php echo esc_attr( $classes ); ?>">
					<span>
						<?php
						$text = '%s';
						esc_html( printf( $text, $value == - 1 ? goso_woo_translate_text( 'goso_woo_trans_all' ) : $value ) );
						?>
					</span>
                </a>
			<?php endforeach; ?>
	...
	public function product_style_select( $is_element ) {
		...
		$shop_view        = get_theme_mod( 'goso_shop_product_view', 'grid_list' );
		$per_row_selector = get_theme_mod( 'goso_woo_per_row_columns_selector', true );
		$columns_settings = get_theme_mod( 'goso_shop_product_columns', 3 );

		$per_row_options = ( 4 >= $columns_settings ) ? array( 2, 3, 4 ) : array( 2, 3, 4, 5, 6 );

		$current_shop_view        = goso_get_shop_view();
		$current_per_row          = goso_get_products_columns_per_row();
		...
		?>
        <div class="goso-products-shop-view <?php echo esc_attr( 'products-view-' . $shop_view ); ?>">
			...
			<?php if ( $per_row_selector && $per_row_options ) : ?>
				<?php foreach ( $products_per_row_options as $key => $value ) : ?>
                    ...
                    <a title="<?php echo sprintf( esc_attr__( 'Show %s columns', 'authow' ), $value ); ?>"
	...
	public function login_to_see_price() {
		$restrict_price = get_theme_mod( 'goso_woocommerce_restrict_cart_price', false );
	...
	public function login_to_see_price_html() {
		$restrict_price = get_theme_mod( 'goso_woocommerce_restrict_cart_price', false );
		if ( ! is_user_logged_in() && $restrict_price ) {
			return '<a href="' . esc_url( get_permalink( wc_get_page_id( 'myaccount' ) ) ) . '" class="restrict-price-tag">' . goso_woo_translate_text( 'goso_woo_trans_lgtspr' ) . '</a>';
		}
	}
	...
	public function top_catalog_product_filter() {
        ...
        <div id="goso-top-product-filter" class="goso-top-product-filter">
			<?php dynamic_sidebar( 'goso-shop-sidebar' ); ?>
        </div>
	...
	public function top_catalog_product_filter_trigger() {
		...
		?>
        <div class="goso-product-top-filter-button">
            <a class="goso-filter-button"
               title="<?php echo goso_woo_translate_text( 'goso_woo_trans_sidebar_filter' ); ?>"
               href="#goso-top-product-filter">
                <i>
                ...
                <span><?php echo goso_woo_translate_text( 'goso_woo_trans_sidebar_filter' ); ?></span>
            </a>
        </div>
		<?php
	}
	...
	public function ajax_load_products() {
		...
		if ( ! wp_verify_nonce( $nonce, 'ajax-nonce' ) ) {
			die( __( 'Ooppppp', 'authow' ) );
		} elseif ( ! empty( $_GET['settings'] ) ) {
			goso_elementor_products_template( $_GET['settings'], $preloader );
			die();
		}
	}
	...
	public function product_sale_countdown() {
		...
		if ( $product->get_type() == 'variable' ) {
			// Variations cache
			$cache                = apply_filters( 'gosocountdown_variable_cache', true );
			$transient_name       = 'gosocountdown_variable_cache_' . $product->get_id();
			...
			if ( ! $available_variations ) {
				...
				if ( $cache ) {
					set_transient( $transient_name, $available_variations, apply_filters( 'gosocountdown_variable_cache_time', WEEK_IN_SECONDS ) );
				}
			}
		...
		if ( apply_filters( 'gosowp_timezone', false ) ) {
			$timezone = wc_timezone_string();
		}

		wp_enqueue_script( 'woo-countdown' );

		echo '<div class="goso-product-countdown goso-time-countdown" data-end-date="' . esc_attr( date( 'Y-m-d H:i:s', $sale_date_end ) ) . '" data-timezone="' . $timezone . '"></div>';
	}
	...
	public function mobile_menu_filter( $items, $args ) {
		...
		if ( goso_is_shop_catelog() && $args->menu == $menu_id ) {
			$items .= '<li class="cart-filter"><a href="#sidebar-product-filter"><i class="fa fa-filter" aria-hidden="true"></i>' . goso_woo_translate_text( 'goso_woo_trans_fasort' ) . '</a></li>';
		}
	...
	public function empty_cart_message() {
        ...
        <div class="woocommerce-content-wrapper">
            <div class="goso-empty-cart goso-empty-page goso-empty-page-text">
                <h3 class="goso-compare-empty-title"><?php echo goso_woo_translate_text( 'goso_woo_cart_empty_title' ) ?></h3>
				<?php echo goso_woo_translate_text( 'goso_woo_cart_empty_textarea' ) ?>
            </div>
        </div>
		<?php
	}
	...
	public function mobile_cart_totals() {
		echo '<span class="goso-mobile-cart-item current-item">' . WC()->cart->get_cart_contents_count() . '</span>';
	}
	...
	public function mobile_cart_totals_fragment( $fragments ) {
		...
		?>
        <span class="goso-mobile-cart-item current-item"><?php echo esc_attr( $woocommerce->cart->cart_contents_count ); ?></span>
		<?php
		$fragments['span.goso-mobile-cart-item.current-item'] = ob_get_clean();

		return $fragments;
	}

	public function goso_add_to_cart_variable() {
		ob_start();
	...
	public function blog_search_result() {
		if ( ! is_search() || ! get_theme_mod( 'goso_woocommerce_search_included_posts', true ) ) {
			return;
		}
	...
	public function product_search_result() {
		if ( ! is_search() || ! apply_filters( 'goso_enable_product_post_search', false ) ) {
			return;
		}
		$search_query = get_search_query();
		goso_elementor_products_template( [ 'search_query' => $search_query ] );
	}
}

$goso_product_misc = new goso_product_misc;
