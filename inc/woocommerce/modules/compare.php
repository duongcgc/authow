...
class penci_product_compare {
	...
	function add_to_compare() {
		...
		if ( $productID && 'add' == $action ) {
			...
			$return['notify'] = sprintf( esc_html__( 'The product %s successfully added to compare table.', 'authow' ), get_the_title( $productID ) );
		} elseif ( $productID && 'remove' == $action ) {
			$this->remove_item_to_compare( $this->save_compare, $productID );
			$return['notify'] = sprintf( esc_html__( 'The product %s successfully removed to compare table.', 'authow' ), get_the_title( $productID ) );
		} else {
			$return = array(
				'message' => 'error',
				'url'     => get_home_url(),
