...
$number_posts         = get_theme_mod( 'goso_home_popular_post_numberposts' ) ? get_theme_mod( 'goso_home_popular_post_numberposts' ) : 10;
$popular_title_length = get_theme_mod( 'goso_home_polular_title_length' ) ? get_theme_mod( 'goso_home_polular_title_length' ) : 8;
$query                = array(
...
if ( get_theme_mod( 'goso_home_popular_type' ) == 'week' ) {
	$query = array(
		'posts_per_page' => $number_posts,
		'post_type'      => 'post',
...
} elseif ( get_theme_mod( 'goso_home_popular_type' ) == 'month' ) {
	$query = array(
		'posts_per_page' => $number_posts,
		'post_type'      => 'post',
...
$popular_cat = get_theme_mod( 'goso_home_popular_cat' );
...
if ( $popular->have_posts() ) {
	$data_loop            = '';
	$data_nav             = get_theme_mod( 'goso_home_popular_shownav' ) ? 'true' : 'false';
	$data_dots            = get_theme_mod( 'goso_home_popular_hidedots' ) ? 'false' : 'true';
	$number_posts_display = $popular->post_count;
	if ( $number_posts_display < 5 ):
		$data_loop = ' data-loop="false"';
	endif;
	$popular_title = get_theme_mod( 'goso_home_popular_title' );
	?>
    <div class="container goso-home-popular-posts">
        <h2 class="home-pupular-posts-title">
		<span>
			<?php echo goso_get_setting( 'goso_home_popular_title' ); ?>
		</span>
        </h2>
			...
			<?php while ( $popular->have_posts() ) : $popular->the_post(); ?>
                <div class="item-related">
                    ...
                    <a class="related-thumb goso-image-holder <?php echo goso_classes_slider_lazy(); ?>"
                       ...
                       data-src="<?php echo goso_get_featured_image_size( get_the_ID(), goso_featured_images_size() ); ?>">
						<?php } else { ?>
