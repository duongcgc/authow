    ...
    <div class="item">
        <div class="wrapper-item wrapper-item-classess">
			...
			while ( $feat_query->have_posts() ) : $feat_query->the_post();
				$thumb = $image_size;
				if ( ( $i % 3 == 1 ) && ! goso_is_mobile() ): $thumb = $image_size_big; endif;
				?>
                <div class="goso-item-mag goso-item-<?php echo( $i % 3 ); ?>">
					<?php if ( ! get_theme_mod( 'goso_disable_lazyload_slider' ) ) { ?>
                        <a class="goso-image-holder <?php echo goso_classes_slider_lazy(); ?>"
                           data-bgset="<?php echo goso_image_srcset( get_the_ID(),$thumb,$post_thumb_msize ); ?>"
                           href="<?php the_permalink(); ?>"
                           title="<?php echo wp_strip_all_tags( get_the_title() ); ?>"></a>
					<?php } else { ?>
                        <a class="goso-image-holder"
                           style="background-image: url('<?php echo goso_get_featured_image_size( get_the_ID(), goso_is_mobile() ? $post_thumb_msize : $thumb ); ?>');"
                           href="<?php the_permalink(); ?>"
                           title="<?php echo wp_strip_all_tags( get_the_title() ); ?>"></a>
					<?php } ?>
                    <div class="goso-slide-overlay goso-slider6-overlay">
                        ...
                        <div class="goso-mag-featured-content">
                            <div class="feat-text<?php if ( get_theme_mod( 'goso_featured_meta_date' ) ): echo ' slider-hide-date'; endif; ?>">
								<?php if ( $i % 3 == 1 || get_theme_mod( 'goso_featured_show_categories' ) ): ?>
