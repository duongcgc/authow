...
class InstgramSettingPages {
	...
	public function add_settings_page() {
		add_submenu_page(
			'authow_dashboard_welcome',
			esc_html__( 'Connect Instagram', 'authow' ),
			esc_html__( 'Connect Instagram', 'authow' ),
			'manage_options',
			'goso_instgram_token',
	...
	public function sb_instagram_feed() {
		...
		if ( ! empty( $_GET['page'] ) && sanitize_text_field( $_GET['page'] ) === 'sb-instagram-feed' ) {
			...
			if ( ! empty( $_GET['access_token'] ) ) {
				...
				update_option( 'goso_options[goso_instagram]', $account );
			}

			// Redirect
			$redirect = admin_url( 'admin.php?page=goso_instgram_token' );
	...
	public function dashboard_content() {
		...
		$instagram_token       = get_option( 'goso_options[goso_instagram]' );
		$instagram_label       = __( 'You\'ve not connected to any Instagram Account.', 'authow' );
		$instagram_description = sprintf( __( 'You can <a class="%1$s" href="%2$s" target="_blank">click here</a> to connect to your Instagram account.', 'authow' ), 'goso_instagram_access_token instagram', $instagram_api );
		if ( is_array( $instagram_token ) && ! empty( $instagram_token ) ) {
			$instagram_label       = sprintf( __( 'Connected to account <strong>%s</strong>', 'authow' ), $instagram_token['username'] );
			$instagram_description = sprintf( __( 'This token is valid until <strong>%1$s</strong> because Instagram just provide an active token in 60 days.<br>Before it expired, you should connect it again to make it won\'t break the connection to Instagram.<br>You can re-connect or connect to another Instagram account by click <a class="%2$s" href="%3$s" target="_blank">HERE</a>.', 'authow' ), date( 'F d, Y H:i:s', (int) $instagram_token['expires_on'] ), 'goso_instagram_access_token instagram', $instagram_api );
		}
		?>
