...
class InstgramSettingPages {
	...
	public function dashboard_content() {
		...
		$instagram_description = sprintf( __( 'You can <a class="%1$s" href="%2$s" target="_blank">click here</a> to connect to your Instagram account.', 'authow' ), 'penci_instagram_access_token instagram', $instagram_api );
		if ( is_array( $instagram_token ) && ! empty( $instagram_token ) ) {
			...
			$instagram_description = sprintf( __( 'This token is valid until <strong>%1$s</strong> because Instagram just provide an active token in 60 days.<br>Before it expired, you should connect it again to make it won\'t break the connection to Instagram.<br>You can re-connect or connect to another Instagram account by click <a class="%2$s" href="%3$s" target="_blank">HERE</a>.', 'authow' ), date( 'F d, Y H:i:s', (int) $instagram_token['expires_on'] ), 'penci_instagram_access_token instagram', $instagram_api );
		}
		?>
