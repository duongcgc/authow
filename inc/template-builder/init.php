...
class GosoTemplateBuilder {
	...
	public function register_single_widget( $widgets_manager ) {
		...
		foreach ( $post_elements as $pelement ) {
			...
			$classname  = '\\GosoSingle' . $class_name;
	...
	public function register_archive_widget( $widgets_manager ) {
		...
		foreach ( $archive_elements as $aelement ) {
			...
			$classname = '\\GosoArchive' . ucwords( $aelement );
			$widgets_manager->register( new $classname() );
		}
	...
	public function archive_widget_categories( $elements_manager ) {
		...
		$elements_manager->add_category( $category_prefix . 'custom-archive-builder', [
			'title' => '[GosoDesign] Archive Pages Builder',
			'icon'  => 'fa fa-plug',
		] );

		$elements_manager->add_category( $category_prefix . 'single-builder', [
			'title' => '[GosoDesign] Post Pages Builder',
			'icon'  => 'fa fa-plug',
		] );
	...
	public function post_type() {
		...
		register_post_type( 'archive-template', array(
			'labels'              => array(
				'name'               => esc_html__( 'Archive Templates', 'authow' ),
				'singular_name'      => esc_html__( 'Archive Template', 'authow' ),
				'menu_name'          => esc_html__( 'Archive Templates', 'authow' ),
				'add_new'            => esc_html__( 'New Archive Template', 'authow' ),
				'add_new_item'       => esc_html__( 'Build Archive Template', 'authow' ),
				'edit_item'          => esc_html__( 'Edit Archive Template', 'authow' ),
				'new_item'           => esc_html__( 'New Archive Template Entry', 'authow' ),
				'view_item'          => esc_html__( 'View Archive Template', 'authow' ),
				'search_items'       => esc_html__( 'Search Archive Templates', 'authow' ),
				'not_found'          => esc_html__( 'No template found', 'authow' ),
				'not_found_in_trash' => esc_html__( 'No Archive Template in Trash', 'authow' ),
				'parent_item_colon'  => ''
			),
			'description'         => esc_html__( 'Single Archive Template', 'authow' ),
			'public'              => true,
			'show_ui'             => true,
			'menu_position'       => 8,
		...
		register_post_type( 'custom-post-template', array(
			'labels'              => array(
				'name'               => esc_html__( 'Post Templates', 'authow' ),
				'singular_name'      => esc_html__( 'Post Template', 'authow' ),
				'menu_name'          => esc_html__( 'Post Templates', 'authow' ),
				'add_new'            => esc_html__( 'New Post Template', 'authow' ),
				'add_new_item'       => esc_html__( 'Build Custom Post Template', 'authow' ),
				'edit_item'          => esc_html__( 'Edit Post Template', 'authow' ),
				'new_item'           => esc_html__( 'New Custom Post Template Entry', 'authow' ),
				'view_item'          => esc_html__( 'View Custom Post Template', 'authow' ),
				'search_items'       => esc_html__( 'Search Custom Post Templates', 'authow' ),
				'not_found'          => esc_html__( 'No template found', 'authow' ),
				'not_found_in_trash' => esc_html__( 'No Custom Post Template in Trash', 'authow' ),
				'parent_item_colon'  => ''
			),
			'description'         => esc_html__( 'Custom Single Post Template', 'authow' ),
			'public'              => true,
...
GosoTemplateBuilder::getInstance();
