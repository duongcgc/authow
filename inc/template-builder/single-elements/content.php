...
class GosoSingleContent extends \Elementor\Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Post - Content', 'authow' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'block_style',
			[
				'label'   => esc_html__( 'Blockquote Style', 'authow' ),
				'description'   => esc_html__( 'blockquote styles just applies when you use Classic Block or Classic Editor', 'authow' ),
		...
		$this->add_group_control(
			\Elementor\Group_Control_Typography::get_type(), array(
				'name'     => 'heading_typo',
				'label'    => __( 'Typography for General Content', 'authow' ),
		...
		for ( $i = 1; $i <= 6; $i ++ ) {
			$this->add_group_control(
				\Elementor\Group_Control_Typography::get_type(), array(
					...
					'label'    => sprintf( __( 'Typography for H%s', 'authow' ), $i ),
