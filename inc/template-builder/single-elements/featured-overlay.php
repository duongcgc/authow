...
class GosoSingleFeaturedOverlay extends \Elementor\Widget_Base {
	...
	protected function register_controls() {
		...
		$this->add_control( 'mobile_breadcrumb', [
			...
			'type'      => \Elementor\Controls_Manager::SWITCHER,
			'selectors' => [ '(mobile) {{WRAPPER}} .container.penci-breadcrumb' => 'display:none' ]
		] );

		$this->add_control( 'mobile_categories', [
			'label'     => esc_html__( 'Hide Categories on Mobile', 'authow' ),
			'type'      => \Elementor\Controls_Manager::SWITCHER,
			'selectors' => [ '(mobile) {{WRAPPER}} .penci-single-cat' => 'display:none' ]
		] );

		$this->add_control( 'mobile_stitle', [
			'label'     => esc_html__( 'Hide Sub Title on Mobile', 'authow' ),
			'type'      => \Elementor\Controls_Manager::SWITCHER,
			'selectors' => [ '(mobile) {{WRAPPER}} .penci-psub-title' => 'display:none' ]
		] );
		...
		$this->add_group_control( \Elementor\Group_Control_Typography::get_type(), array(
			...
			'selector' => '{{WRAPPER}} .penci-breadcrumb.single-breadcrumb, {{WRAPPER}} .penci-breadcrumb.single-breadcrumb span, {{WRAPPER}} .penci-breadcrumb.single-breadcrumb a',
		) );
		...
		$this->add_group_control( \Elementor\Group_Control_Typography::get_type(), array(
			...
			'selector' => '{{WRAPPER}} .cat > .penci-cat-name',
		) );

		$this->add_control( 'cat_color', [
			'label'     => 'Categories Text Color',
			'type'      => \Elementor\Controls_Manager::COLOR,
			'selectors' => [ '{{WRAPPER}} .cat > .penci-cat-name, {{WRAPPER}} .cat' => 'color:{{VALUE}}' ],
		] );
	...
	protected function render() {
		...
		if ( has_post_thumbnail() ) {
			...
			if ( $enable_jarallax ) {
				...
				$image_html = '<span class="' . $class . ' penci-disable-lazy" style="background-image: url(' . $src . ');"></span>';
			} else {
				$image_html = '<span class="' . $class . ' penci-lazy" data-bgset="' . $src . '"></span>';
			}
		}
