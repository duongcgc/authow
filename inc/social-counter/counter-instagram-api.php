...
if ( ! class_exists( 'Goso_Social_Counter_Instagram_API' ) ):
	class Goso_Social_Counter_Instagram_API {
		public static function get_count( $data, $cache_period ) {
			...
			$data['icon'] = goso_icon_by_ver( 'fab fa-instagram' );

			$cache_key       = 'goso_counter__instagram' . $user_id;
			$default_count   = goso_get_social_counter_option( 'instagram_default' );
			$instagram_count = $default_count ? $default_count : get_transient( $cache_key );
			if ( ! $instagram_count ) {

				$count = self::goso_get_instagram_data( $user_id );
		...
		public static function goso_get_instagram_data( $username ) {
