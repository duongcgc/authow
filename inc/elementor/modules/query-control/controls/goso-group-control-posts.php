...
namespace GosoAuthowElementor\Modules\QueryControl\Controls;

use Elementor\Controls_Manager;
use Elementor\Group_Control_Base;
use GosoAuthowElementor\Classes\Utils;

if ( ! defined( 'ABSPATH' ) ) {
	exit; // Exit if accessed directly
}

class Goso_Group_Control_Posts extends Group_Control_Base {
	...
	final protected function init_fields() {
		...
		$fields['post_type'] = array(
			'label' => __( 'Source', 'authow' ),
			'type'  => Controls_Manager::SELECT
		);

		$fields['related_post_by'] = array(
			'label'       => __( 'Display Related Posts By', 'authow' ),
			'label_block' => true,
			'type'        => Controls_Manager::SELECT,
			'multiple'    => false,
			'default'     => 'categories',
	...
	final protected function prepare_fields( $fields ) {
		...
		$post_types_options['by_id']         = __( 'Manual Selection', 'authow' );
		$post_types_options['current_query'] = __( 'Archive Builder / Current Query', 'authow' );
		$post_types_options['related_posts'] = __( 'Post Builder - Related Posts', 'authow' );
		...
		$fields['ignore_sticky_posts'] = array(
			...
			'description' => __( 'Sticky-posts ordering is visible on frontend only. And includes sticky posts doesn\'t work if you filter your posts by a taxonomy or multiple taxonomies (categories, tags... ) - because it doesn\'t support by WordPress itself.', 'authow' ),
			'separator'   => 'before',
		);
