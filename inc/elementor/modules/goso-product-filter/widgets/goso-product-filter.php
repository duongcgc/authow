...
namespace GosoAuthowElementor\Modules\GosoProductFilter\Widgets;

use Elementor\Controls_Manager;
use Elementor\Repeater;
use Goso_Custom_Walker_Category;
 ...
 * @since 1.0.0
 */
class GosoProductFilter extends Base_Widget {
	/**
	 * Get widget name.
	 *
	...
	protected function register_controls() {
		...
		$repeater->add_control(
			'filter_type',
			[
				'label'   => esc_html__( 'Filter type', 'authow' ),
				'type'    => Controls_Manager::SELECT,
				'options' => [
					'categories' => esc_html__( 'Categories', 'authow' ),
					'attributes' => esc_html__( 'Attributes', 'authow' ),
					'stock'      => esc_html__( 'Stock status', 'authow' ),
					'price'      => esc_html__( 'Price', 'authow' ),
					'orderby'    => esc_html__( 'Order by', 'authow' ),
				],
				'default' => 'categories',
			]
		);
		...
		$repeater->add_control(
			...
			[
				...
				'options'   => array(
					...
					'slug'  => esc_html__( 'Slug', 'authow' ),
					'count' => esc_html__( 'Count', 'authow' ),
					'order' => esc_html__( 'Category order', 'authow' ),
				),
				'condition' => [
					'filter_type' => 'categories',
				],
		...
		$repeater->add_control(
			'hierarchical',
			[
				'label'        => esc_html__( 'Show hierarchy', 'authow' ),
		...
		$repeater->add_control(
			'hide_empty',
			[
				'label'        => esc_html__( 'Hide empty categories', 'authow' ),
				'type'         => Controls_Manager::SWITCHER,
				'default'      => '0',
				'label_on'     => esc_html__( 'Yes', 'authow' ),
				'label_off'    => esc_html__( 'No', 'authow' ),
		...
		$repeater->add_control(
			...
			[
				'label'        => esc_html__( 'Show current category ancestors', 'authow' ),
				'description'  => esc_html__( 'If you visit category Man, for example, only man\'s subcategories will be shown in the page title like T-shirts, Coats, Shoes etc.', 'authow' ),
				'type'         => Controls_Manager::SWITCHER,
				'default'      => '0',
				'label_on'     => esc_html__( 'Yes', 'authow' ),
				'label_off'    => esc_html__( 'No', 'authow' ),
		...
		$repeater->add_control(
			'attribute',
			[
				'label'     => esc_html__( 'Attribute', 'authow' ),
				'type'      => Controls_Manager::SELECT,
		...
		$repeater->add_control(
			'size',
			[
				'label'     => esc_html__( 'Swatches size', 'authow' ),
				'type'      => Controls_Manager::SELECT,
				'default'   => 'normal',
				'options'   => array(
					'small'  => esc_html__( 'Small (15px)', 'authow' ),
					'normal' => esc_html__( 'Normal (25px)', 'authow' ),
					'large'  => esc_html__( 'Large (35px)', 'authow' ),
				),
				'condition' => [
					'filter_type' => 'attributes',
				],
		...
		$repeater->add_control(
			'labels',
			[
				'label'        => esc_html__( 'Show labels', 'authow' ),
				'type'         => Controls_Manager::SWITCHER,
				'default'      => '1',
				'label_on'     => esc_html__( 'Yes', 'authow' ),
				'label_off'    => esc_html__( 'No', 'authow' ),
		...
		$repeater->add_control(
			'onsale',
			[
				'label'        => esc_html__( 'On Sale filter', 'authow' ),
				'type'         => Controls_Manager::SWITCHER,
				'default'      => '1',
				'label_on'     => esc_html__( 'Yes', 'authow' ),
				'label_off'    => esc_html__( 'No', 'authow' ),
		...
		$repeater->add_control(
			'instock',
			[
				'label'        => esc_html__( 'In Stock filter', 'authow' ),
				'type'         => Controls_Manager::SWITCHER,
				'default'      => '1',
				'label_on'     => esc_html__( 'Yes', 'authow' ),
				'label_off'    => esc_html__( 'No', 'authow' ),
		...
		$this->add_control(
			...
			[
				...
				'options' => [
					...
					'light' => esc_html__( 'Light', 'authow' ),
					'dark'  => esc_html__( 'Dark', 'authow' ),
				],
				'default' => '',
			]
		);
	...
	public function categories_filter_template( $settings ) {
		...
		$default_settings = [
			'categories_title'          => esc_html__( 'Categories', 'authow' ),
			'hierarchical'              => 1,
			'order_by'                  => 'name',
			'hide_empty'                => '',
			'show_categories_ancestors' => '',
		...
		$list_args = [
			...
			'title_li'           => false,
			'walker'             => new Goso_Custom_Walker_Category(),
	...
	public function attributes_filter_template( $settings ) {
		$default_settings = [
			'attributes_title' => esc_html__( 'Filter by', 'authow' ),
			'attribute'        => '',
			'categories'       => '',
			'query_type'       => 'and',
			'size'             => 'normal',
	...
	public function stock_filter_template( $settings ) {
		$default_settings = [
			'stock_title' => esc_html__( 'Stock status', 'authow' ),
			'instock'     => 1,
			'onsale'      => 1,
		];
					...
					<?php if ( $settings['onsale'] ) : ?>
                        <li>
							<span class="pf-value" data-val="onsale"
                                  data-title="<?php esc_html_e( 'On sale', 'authow' ); ?>">
								<?php esc_html_e( 'On sale', 'authow' ); ?>
							</span>
                        </li>
					<?php endif; ?>
					...
					<?php if ( $settings['instock'] ) : ?>
                        <li>
							<span class="pf-value" data-val="instock"
                                  data-title="<?php esc_html_e( 'In stock', 'authow' ); ?>">
								<?php esc_html_e( 'In stock', 'authow' ); ?>
							</span>
                        </li>
					<?php endif; ?>
                </ul>
	...
	public function price_filter_template( $settings ) {
		$default_settings = [
			'price_title' => esc_html__( 'Filter by price', 'authow' ),
		];
