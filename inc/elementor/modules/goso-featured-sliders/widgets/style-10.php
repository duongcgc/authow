...
$goso_is_mobile = goso_is_mobile();
	...
	$post_thumb_size_m = ! empty( $post_thumb_size_mobile ) ? $post_thumb_size_mobile : 'goso-masonry-thumb';
                ...
                <div class="goso-item-mag goso-item-<?php echo( $i % 3 ); ?>">
					<?php if ( ! $disable_lazyload ) { ?>
                        <a class="goso-image-holder <?php echo goso_classes_slider_lazy(); ?>"
                           data-bgset="<?php echo goso_image_srcset( get_the_ID(),$post_thumb_size,$post_thumb_size_m ); ?>"
                           href="<?php the_permalink(); ?>"
                           title="<?php echo wp_strip_all_tags( get_the_title() ); ?>"></a>
					<?php } else { ?>
                        <a class="goso-image-holder"
                           style="background-image: url('<?php echo goso_get_featured_image_size( get_the_ID(), $goso_is_mobile ? $post_thumb_size_m : $post_thumb_size ); ?>');"
                           href="<?php the_permalink(); ?>"
                           title="<?php echo wp_strip_all_tags( get_the_title() ); ?>"></a>
					<?php } ?>
