...
class GosoImageGallery extends Base_Widget {
	...
	public function get_title() {
		return penci_get_theme_name('Goso').' '.esc_html__( ' Image Gallery', 'authow' );
	}
			...
			<div class="penci-block_content <?php echo esc_attr( 's1' == $style_gallery ? ' pencisc-grid' : '' ); ?>">
				...
				if ( 's2' == $style_gallery ) {
					foreach ( $images as $image_key => $image_item ) {
						...
						echo \Goso_Vc_Helper::get_image_holder_gal( $image_item, $penci_img_size_pre, $penci_img_type, true, $gal_count, $class_item, $settings['caption_source'] );
					...
					foreach ( $images as $image_key => $image_item ) {
						...
						echo \Goso_Vc_Helper::get_image_holder_gal( $image_item, $penci_img_size, $penci_img_type, true, $gal_count, $class_item, $settings['caption_source'] );
					...
					foreach ( $images as $image_key => $image_item ) {
						...
						echo \Goso_Vc_Helper::get_image_holder_gal( $image_item, $penci_img_size, $penci_img_type, true, $gal_count, '', $settings['caption_source'] );
					}
				}
				?>
			</div>
