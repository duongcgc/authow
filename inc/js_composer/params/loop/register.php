...
function penci_authow_vc_param_loop( $settings, $value ) {
	...
	return '<div class="vc_loop">'
	       ...
	       . '<a href="#" class="button vc_loop-build ' . $settings['param_name'] . '_button" data-settings="' . rawurlencode( json_encode( $settings['settings'] ) ) . '">' . __( 'Build query', 'authow' ) . '</a>'
...
if( ! class_exists( 'GosoAuthowLoopQueryBuilder' ) ):
class GosoAuthowLoopQueryBuilder {
	...
	protected function parse_tax_query( $value ) {
		...
		$terms = get_terms( GosoAuthowLoopSettings::getTaxonomies(),
...
if( ! class_exists( 'GosoAuthowLoopSettings' ) ):
class GosoAuthowLoopSettings {
	...
	function __construct( $value, $settings = array() ) {
		$this->parts    = array(
			'size'       => __( 'Post count', 'authow' ),
			'offset'     => __( 'Post offset', 'authow' ),
			'order_by'   => __( 'Order by', 'authow' ),
			'order'      => __( 'Sort order', 'authow' ),
			'post_type'  => __( 'Post types', 'authow' ),
			'authors'    => __( 'Author', 'authow' ),
			'categories' => __( 'Categories', 'authow' ),
			'tags'       => __( 'Tags', 'authow' ),
			'tax_query'  => __( 'Taxonomies', 'authow' ),
			'by_id'      => __( 'Individual posts/pages', 'authow' ),
		);
	...
	public function parse_order_by( $value ) {
		return $this->parseDropDown( $value, array(
			array( 'date', __( 'Date', 'authow' ) ),
			array( 'modified', __( 'Latest Modified', 'authow' ) ),
			array( 'rand', __( 'Random', 'authow' ) ),
			array( 'random_today', __( 'Random Today', 'authow' ) ),
			array( 'random_7_day', __( 'Random from last 7 days', 'authow' ) ),
			array( 'alphabetical_order', __( 'Alphabetical ( A - Z )', 'authow' ) ),
			array( 'popular', __( 'Popular posts in all time', 'authow' ) ),
			array( 'popular_month', __( 'Popular posts in once month', 'authow' ) ),
			array( 'popular7', __( 'Popular posts in once week', 'authow' ) ),
			array( 'ID', __( 'ID', 'authow' ) ),
			array( 'author', __( 'Author', 'authow' ) ),
			array( 'title', __( 'Title', 'authow' ) ),
			array( 'comment_count', __( 'Comment count', 'authow' ) ),
			array( 'featured_product', __( 'Featured ( Woo )', 'authow' ) ),
		) );
	}

	public function parse_order( $value ) {
		return $this->parseDropDown( $value, array(
			array( 'ASC', __( 'Ascending', 'authow' ) ),
			array( 'DESC', __( 'Descending', 'authow' ) ),
		) );
	}
	...
	public static function buildWpQuery( $query, $exclude_id = false ) {
		...
		$query_builder = new GosoAuthowLoopQueryBuilder( $data );
		if ( $exclude_id ) {
...
if( ! class_exists( 'GosoAuthowLoopSuggestions' ) ):
class GosoAuthowLoopSuggestions {
	protected $content = array();
	...
	public function get_tax_query( $query ) {
		...
		$tags = get_terms( GosoAuthowLoopSettings::getTaxonomies(), $args );
		foreach ( $tags as $tag ) {
			$this->content[] = array(
				'value' => $tag->term_id,
...
if ( ! function_exists( 'penci_get_loop_suggestion' ) ) {
	function penci_get_loop_suggestion() {
		...
		$loop_suggestions = new GosoAuthowLoopSuggestions( vc_post_param( 'field' ), vc_post_param( 'query' ), vc_post_param( 'exclude' ) );
...
if ( ! function_exists( 'penci_get_loop_settings_json' ) ) {
	function penci_get_loop_settings_json() {
		...
		$loop_settings = new GosoAuthowLoopSettings( vc_post_param( 'value' ), vc_post_param( 'settings' ) );
