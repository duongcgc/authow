        ...
        <div class="wrapper-item wrapper-item-classess">
			...
			while ( $feat_query->have_posts() ) : $feat_query->the_post();
				...
				if ( $i % 5 == 3 ): $thumb = $bpost_thumb_size; endif;
					...
					$image_size_m = ! empty( $post_thumb_size_mobile ) ? $post_thumb_size_mobile : 'goso-masonry-thumb';
                ...
                <div class="goso-item-mag goso-item-<?php echo( $i % 5 ); ?> <?php echo( $i % 5 == 3 ? 'goso-pitem-big' : 'goso-pitem-small' ) ?>">
					<?php if ( ! $disable_lazyload ) { ?>
                        <a class="goso-image-holder <?php echo goso_classes_slider_lazy(); ?>"
                           data-bgset="<?php echo goso_image_srcset( get_the_ID(),$thumb,$image_size_m ); ?>"
                           href="<?php the_permalink(); ?>"
                           title="<?php echo wp_strip_all_tags( get_the_title() ); ?>"></a>
					<?php } else { ?>
                        <a class="goso-image-holder"
                           style="background-image: url('<?php echo goso_get_featured_image_size( get_the_ID(), $goso_is_mobile ? $image_size_m : $thumb ); ?>');"
                           href="<?php the_permalink(); ?>"
                           title="<?php echo wp_strip_all_tags( get_the_title() ); ?>"></a>
					<?php } ?>
