...
$block_id = Goso_Vc_Helper::get_unique_id_block( 'news-ticker' );
...
?>
	<div class="goso-enews-ticker goso-topbar-trending" id="<?php echo esc_attr( $block_id ) ?>">	
		<?php if( $toptext ) {
...
if ( 'yes' == $atts['navs_buttons'] ) {
	$css_custom .= $block_id_css . ' .goso-trending-nav a{ height: 24px; line-height: 24px; top: 4px; background: #313131; color: #fff; float: left; }';
	$css_custom .= $block_id_css . ' .goso-trending-nav a:first-child{ margin-right: 4px; }';
}
if ( 'yes' == $atts['move_navs'] ) {
	$css_custom .= $block_id_css . '.goso-topbar-trending .goso-trending-nav{ float: right; }';
	...
	$css_custom .= 'body.rtl ' . $block_id_css . '.goso-topbar-trending .goso-trending-nav{ float: left; }';
...
if ( 'yes' == $atts['titles_upper'] ) {
	$css_custom .= $block_id_css . ' a.goso-topbar-post-title{ text-transform: none; }';
}
if ( ! empty( $atts['tpost_size'] ) ) {
	$css_custom .= $block_id_css . ' .headline-title{ font-size:' . esc_attr( $atts['tpost_size'] ) . '; }';
}
if ( ! empty( $atts['navs_size'] ) ) {
	$css_custom .= $block_id_css . ' .goso-trending-nav a{ font-size:' . esc_attr( $atts['navs_size'] ) . '; }';
}
if ( ! empty( $atts['ptitles_size'] ) ) {
	$css_custom .= $block_id_css . ' a.goso-topbar-post-title{ font-size:' . esc_attr( $atts['ptitles_size'] ) . '; }';
}
if ( ! empty( $atts['border_color'] ) ) {
	$css_custom .= $block_id_css . '.goso-topbar-trending{ border: 1px solid ' . esc_attr( $atts['border_color'] ) . '; }';
}
if ( ! empty( $atts['bg_color'] ) ) {
	$css_custom .= $block_id_css . '.goso-topbar-trending,'. $block_id_css .' .goso-owl-carousel .owl-item{ background-color: ' . esc_attr( $atts['bg_color'] ) . '; }';
...
if ( ! empty( $atts['navs_color'] ) ) {
	$css_custom .= $block_id_css . ' .goso-trending-nav a{ color: ' . esc_attr( $atts['navs_color'] ) . '; }';
}
if ( ! empty( $atts['navs_hcolor'] ) ) {
	$css_custom .= $block_id_css . ' .goso-trending-nav a:hover{ color: ' . esc_attr( $atts['navs_hcolor'] ) . '; }';
}
if ( ! empty( $atts['navs_bg'] ) ) {
	$css_custom .= $block_id_css . ' .goso-trending-nav a{ background-color: ' . esc_attr( $atts['navs_bg'] ) . '; }';
}
if ( ! empty( $atts['navs_hbg'] ) ) {
	$css_custom .= $block_id_css . ' .goso-trending-nav a:hover{ background-color: ' . esc_attr( $atts['navs_hbg'] ) . '; }';
}
if ( ! empty( $atts['ptitle_color'] ) ) {
	$css_custom .= $block_id_css . ' a.goso-topbar-post-title{ color: ' . esc_attr( $atts['ptitle_color'] ) . '; }';
}
if ( ! empty( $atts['ptitle_hcolor'] ) ) {
	$css_custom .= $block_id_css . ' a.goso-topbar-post-title:hover{ color: ' . esc_attr( $atts['ptitle_hcolor'] ) . '; }';
}

if( $auto_speed && '300' != $auto_speed ){
	$auto_speed_num = (int)$auto_speed / 1000;
	$css_custom .= $block_id_css . '.goso-topbar-trending .animated.slideOutUp,'. $block_id_css .'.goso-topbar-trending .animated.slideInUp,'. $block_id_css .'.goso-topbar-trending .animated.TickerslideOutRight,'. $block_id_css .'.goso-topbar-trending .animated.TickerslideInRight,'. $block_id_css .'.goso-topbar-trending .animated.fadeOut,'. $block_id_css .'.goso-topbar-trending .animated.fadeIn{ -webkit-animation-duration : '. $auto_speed_num .'s; animation-duration : '. $auto_speed_num .'s; }';
}
