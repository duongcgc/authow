...
$css_class .= ' goso-style-' . $cup_style;
$css_class .= ' goso-counterup-' . $cup_align;
$css_class .= ' ' . apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, $class_to_filter, $this->settings['base'], $atts );

$block_id = Goso_Vc_Helper::get_unique_id_block( 'counter_up' );
?>
	<div id="<?php echo esc_attr( $block_id ); ?>" class="<?php echo esc_attr( $css_class ); ?>">
		...
		<div class="goso-counter-up_inner">
			...
			if ( 'icon' == $cup_icon_type ) {
				...
				if ( $cup_icon ) {
					$icon = '<div class="goso-cup_icon goso-cup_icon--icon">';
					$icon .= '<i class="goso-cup_iconn--i ' . $cup_icon . '"></i>';
					$icon .= '</div>';
					echo $icon;
				}
			} elseif ( $cup_image ) {
				$icon = '<div class="goso-cup_icon goso-cup_icon--image">';
			...
			<div class="goso-cup-info">
				...
				<?php if ( $cup_prefix_number ): ?><span class="goso-cup-label goso-cup-prefix"><?php echo $cup_prefix_number; ?></span><?php endif; ?>
					...
					<?php if ( $cup_suffix_number ): ?><span class="goso-cup-label goso-cup-postfix"><?php echo $cup_suffix_number; ?></span><?php endif; ?>
				</span>
				</div>
				<?php if ( $cup_title ): ?>
					<div class="goso-cup-title"><?php echo $cup_title; ?></div><?php endif; ?>
			</div>
		</div>
	</div>
<?php
...
$css_custom = Goso_Vc_Helper::get_heading_block_css( $id_counter_up, $atts );

if ( 'icon' == $cup_icon_type ) {
	if ( $icon_border_style ) {
		$css_custom .= $id_counter_up . ' .goso-cup_icon{ border-style: ' . ( $icon_border_style ) . ' }';

		if ( $icon_border_radius ) {
			$css_custom .= $id_counter_up . ' .goso-cup_icon{ border-radius: ' . ( $icon_border_radius ) . ' }';
		}
		if ( $icon_border_width ) {
			$css_custom .= $id_counter_up . ' .goso-cup_icon{ border-width: ' . ( $icon_border_width ) . ' }';
		} else {
			$css_custom .= $id_counter_up . ' .goso-cup_icon{ border-width: 1px; }';
		}
	}
}

if ( $icon_padding ) {
	$css_custom .= $id_counter_up . ' .goso-cup_icon{ padding: ' . ( $icon_padding ) . ' }';
}

if ( $_image_width_height ) {
	$css_custom .= $id_counter_up . ' .goso-cup_icon{ width: ' . ( $_image_width_height ) . ';height: ' . ( $_image_width_height ) . ' }';
}
if ( $icon_margin_bottom ) {
	$css_custom .= $id_counter_up . ' .goso-cup_icon{ margin-bottom: ' . ( $icon_margin_bottom ) . ' }';
}

if ( $number_margin_top ) {
	$css_custom .= $id_counter_up . ' .goso-cup-number-wrapper{ margin-top: ' . ( $number_margin_top ) . ' }';
}

if ( $number_padding_lr ) {
	$css_custom .= $id_counter_up . ' .goso-counterup-number{ margin-left: ' . ( $number_padding_lr ) . '; margin-right: ' . ( $number_padding_lr ) . '; }';
}

if ( $title_margin_top ) {
	$css_custom .= $id_counter_up . ' .goso-cup-title{ margin-top: ' . ( $title_margin_top ) . ' }';
}
...
if ( $icon_custom_css_color ) {
	$css_custom .= $id_counter_up . ' .goso-cup_icon{' . ( $icon_custom_css_color ) . '}';
}
...
if ( $number_custom_css_color ) {
	$css_custom .= $id_counter_up . ' .goso-counterup-number{' . ( $number_custom_css_color ) . '}';
}
$css_custom .= Goso_Vc_Helper::vc_google_fonts_parse_attributes( array(
	'font_size'  => $cup_number_size,
	'font_style' => $cup_number_typo,
	'template'   => $id_counter_up . ' .goso-counterup-number{ %s }',
) );

if ( $cup_frefix_color ) {
	$css_custom .= $id_counter_up . ' .goso-cup-label { color: ' . ( $cup_frefix_color ) . ';}';
}
$css_custom .= Goso_Vc_Helper::vc_google_fonts_parse_attributes( array(
	'font_size'  => $cup_frefix_size,
	'font_style' => $cup_frefix_typo,
	'template'   => $id_counter_up . ' .goso-cup-label{ %s }',
) );
...
if ( $title_custom_css_color ) {
	$css_custom .= $id_counter_up . ' .goso-cup-title{' . ( $title_custom_css_color ) . '}';
}
$css_custom .= Goso_Vc_Helper::vc_google_fonts_parse_attributes( array(
	'font_size'  => $cup_title_size,
	'font_style' => $cup_title_typo,
	'template'   => $id_counter_up . ' .goso-cup-title{ %s }',
) );
