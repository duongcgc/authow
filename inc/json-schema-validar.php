<?php
if ( ! class_exists( 'Goso_JSON_Schema_Validator' ) ) {

	/**
	 * JSON Schema object
	 *
	 * Class Goso_JSON_Schema_Validator
	 */
	class Goso_JSON_Schema_Validator {
		...
		public static function newsarticle_data() {
			...
			$schema_return = apply_filters('authow_schema', $schema_markup);
		...
		public static function BreadcrumbList_data() {
			...
			if ( is_home() && ! is_front_page() ) {
				...
				$items[] = array(
					'id'   => $author_url,
					'name' => sprintf(
						esc_html__( '%s %s', 'authow' ),
						esc_html__( 'Author', 'authow' ),
						'<span class="vcard"><a class="url fn n" href="' . $author_url . '" title="' . esc_attr( get_the_author() ) . '" rel="me">' . get_the_author() . '</a></span>'
					)
				);
			} elseif ( is_day() ) {
				$items[] = array( 'id' => home_url( add_query_arg( array(), $wp->request ) ), 'name' => sprintf( esc_html__( '%s %s', 'authow' ), esc_html__( 'Day Archives:', 'authow' ), get_the_date() ) );
			} elseif ( is_month() ) {
				$items[] = array( 'id' => home_url( add_query_arg( array(), $wp->request ) ), 'name' => sprintf( esc_html__( '%s %s', 'authow' ), esc_html__( 'Monthly Archives:', 'authow' ), get_the_date( 'F Y' ) ) );
			} elseif ( is_year() ) {
...
new Goso_JSON_Schema_Validator;
