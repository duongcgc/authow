...
class Goso_Custom_Sidebar {
	...
	public static function initialize() {
		...
		add_action( 'wp_ajax_authow_add_sidebar', array( __CLASS__, 'add_sidebar' ) );
		add_action( 'wp_ajax_authow_remove_sidebar', array( __CLASS__, 'remove_sidebar' ) );
	...
	public static function register_sidebars() {
		...
		$sidebars = get_option( 'authow_custom_sidebars' );
		...
		foreach ( ( array ) $sidebars as $id => $sidebar ) {
			...
			$sidebar['class']         = 'authow-custom-sidebar';
	...
	public static function add_sidebar() {
		...
		if ( empty( $nonce ) ) {
			wp_send_json_error( esc_html__( 'Invalid request.', 'authow' ) );
		} elseif ( empty( $name ) ) {
			wp_send_json_error( esc_html__( 'Missing sidebar name.', 'authow' ) );
		}

		if ( ! wp_verify_nonce( $nonce, 'ajax-nonce' ) ) {
			wp_send_json_error( esc_html__( 'Nonce verification fails.', 'authow' ) );
		}

		// Get  custom sidebars.
		$sidebars    = get_option( 'authow_custom_sidebars', array() );
		$sidebar_num = get_option( 'authow_custom_sidebars_lastid', - 1 );
		...
		update_option( 'authow_custom_sidebars_lastid', ++ $sidebar_num );

		$sidebars[ 'authow-custom-sidebar-' . $sidebar_num ] = array(
			'id'            => 'authow-custom-sidebar-' . $sidebar_num,
		...
		update_option( 'authow_custom_sidebars', $sidebars );
		...
		ob_start();
		?>
        <div class="widgets-holder-wrap sidebar-authow-custom-sidebar closed">
			<?php wp_list_widget_controls( 'authow-custom-sidebar-' . $sidebar_num, stripcslashes( $name ) ); ?>
        </div>
		<?php
		$output = ob_get_clean();
	...
	public static function remove_sidebar() {
		...
		if ( empty( $nonce ) ) {
			...
			wp_send_json_error( esc_html__( 'Missing sidebar ID', 'authow' ) );
		}
		...
		$custom_sidebars = get_option( 'authow_custom_sidebars', array() );

		unset( $custom_sidebars[ $idSidebar ] );

		update_option( 'authow_custom_sidebars', $custom_sidebars );
	...
	public static function admin_page() {
        ...
        <div class="widgets-holder-wrap">
            <div id="penci-add-custom-sidebar" class="widgets-sortables">
                <div class="sidebar-name">
                    ...
                    <h2>
						<?php esc_html_e( 'Add New Sidebar', 'authow' ); ?>
                        <span class="spinner"></span>
                    </h2>
                </div>
                <div class="sidebar-description">
                    <form class="description" method="POST" action="">
						<?php wp_nonce_field( 'authow_add_sidebar' ); ?>
                            ...
                            <tr valign="top">
                                ...
                                <td>
                                    <input type="submit" class="button-primary"
                                           value="<?php esc_attr_e( 'Add', 'authow' ) ?>">
                                </td>
                            </tr>
                        </table>
                    </form>
        ...
        <style type="text/css" media="screen">
			.authow-remove-custom-sidebar .notice-dismiss {
				right: 30px;
				top: 3px;
			}
        </style>
	...
	public static function get_list_sidebar_el() {
		...
		$list_sidebar = array(
			'main-sidebar'      => esc_html__( 'Main Sidebar', 'authow' ),
			'main-sidebar-left' => esc_html__( 'Main Sidebar Left', 'authow' ),
			'custom-sidebar-1'  => esc_html__( 'Custom Sidebar 1', 'authow' ),
			'custom-sidebar-2'  => esc_html__( 'Custom Sidebar 2', 'authow' ),
			'custom-sidebar-3'  => esc_html__( 'Custom Sidebar 3', 'authow' ),
			'custom-sidebar-4'  => esc_html__( 'Custom Sidebar 4', 'authow' ),
			'custom-sidebar-5'  => esc_html__( 'Custom Sidebar 5', 'authow' ),
			'custom-sidebar-6'  => esc_html__( 'Custom Sidebar 6', 'authow' ),
			'custom-sidebar-7'  => esc_html__( 'Custom Sidebar 7', 'authow' ),
			'custom-sidebar-8'  => esc_html__( 'Custom Sidebar 8', 'authow' ),
			'custom-sidebar-9'  => esc_html__( 'Custom Sidebar 9', 'authow' ),
			'custom-sidebar-10' => esc_html__( 'Custom Sidebar 10', 'authow' )
		);
...
Goso_Custom_Sidebar::initialize();