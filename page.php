 ...
 */
get_header();
$breadcrumb      = get_post_meta( get_the_ID(), 'goso_page_breadcrumb', true );
$featured_boxes  = get_post_meta( get_the_ID(), 'goso_page_display_featured_boxes', true );
$page_meta       = get_post_meta( get_the_ID(), 'goso_page_slider', true );
$rev_shortcode   = get_post_meta( get_the_ID(), 'goso_page_rev_shortcode', true );
$sidebar_default = get_theme_mod( 'goso_page_default_template_layout' );
...
if ( $sidebar_default == 'small-width' ):
	$class_small_width = ' goso-page-container-smaller';
endif;
...
if ( in_array( $page_meta, array(
	...
	if ( $page_meta == 'video' && get_theme_mod( 'goso_featured_video_url' ) ) {
		...
		if ( ( $page_meta == 'style-33' || $page_meta == 'style-34' ) && $rev_shortcode ) {
			...
			if ( $page_meta == 'style-7' || $page_meta == 'style-8' ) {
				...
				$data_next_prev = get_theme_mod( 'goso_enable_next_prev_goso_slider' ) ? 'true' : 'false';
				$data_dots      = get_theme_mod( 'goso_disable_dots_goso_slider' ) ? 'false' : 'true';
				$data_res       = ' data-dots="' . $data_dots . '" data-nav="' . $data_next_prev . '"';
			}

			if ( get_theme_mod( 'goso_featured_autoplay' ) ): $data_auto = 'true'; endif;
			if ( get_theme_mod( 'goso_featured_loop' ) ): $data_loop = 'false'; endif;
			$auto_time = get_theme_mod( 'goso_featured_slider_auto_time' );
			...
			if ( get_theme_mod( 'goso_enable_flat_overlay' ) && in_array( $page_meta, array(
					'style-6',
					'style-7',
					'style-8',
					'style-9',
					...
					'style-27',
					'style-28'
				) ) ): $slider_class .= ' goso-flat-overlay'; endif;
			...
			echo '<div class="goso-owl-carousel goso-owl-featured-area"' . $data_res . ' data-style="' . $page_meta . '" data-auto="' . $data_auto . '" data-autotime="' . $auto_time . '" data-speed="' . $auto_speed . '" data-loop="' . $data_loop . '">';
...
if ( $featured_boxes == 'yes' && ! get_theme_mod( 'goso_home_hide_boxes' ) && ( get_theme_mod( 'goso_home_box_img1' ) || get_theme_mod( 'goso_home_box_img2' ) || get_theme_mod( 'goso_home_box_img3' ) || get_theme_mod( 'goso_home_box_img4' ) ) ):
	get_template_part( 'inc/modules/home_boxes' );
endif;

?>
<?php $show_page_title = goso_is_pageheader(); ?>
<?php if ( ! $show_page_title ): ?>
	<?php if ( ! get_theme_mod( 'goso_disable_breadcrumb' ) && ( 'no' != $breadcrumb ) && ! get_theme_mod( 'goso_move_breadcrumbs' ) ): ?>
		<?php
		$yoast_breadcrumb = '';
		if ( function_exists( 'yoast_breadcrumb' ) ) {
			$yoast_breadcrumb = yoast_breadcrumb( '<div class="container container-single-page goso-breadcrumb ' . $class_small_width . '">', '</div>', false );
		}

		if ( $yoast_breadcrumb ) {
			echo $yoast_breadcrumb;
		} else { ?>
            <div class="container container-single-page goso-breadcrumb<?php echo $class_small_width; ?>">
                <span><a class="crumb"
				...
				if ( ! empty( $page_parent ) ):
					...
					foreach ( $page_parent as $pages ) {
                        ...
                        </span><?php goso_fawesome_icon( 'fas fa-angle-right' ); ?>
					<?php }
				endif; ?>
...
if ( in_array( $sidebar_default, array( 'left-sidebar', 'right-sidebar', 'two-sidebar' ) ) ) {
	echo ' goso_sidebar ' . $sidebar_default;
} else {
	echo ' goso_is_nosidebar';
} ?>">
    <div id="main"
         class="goso-main-single-page-default <?php if ( get_theme_mod( 'goso_sidebar_sticky' ) ): ?> goso-main-sticky-sidebar<?php endif; ?>">
        <div class="theiaStickySidebar">
			...
			<?php if ( ! $show_page_title ): ?>
				<?php if ( ! get_theme_mod( 'goso_disable_breadcrumb' ) && ( 'no' != $breadcrumb ) && get_theme_mod( 'goso_move_breadcrumbs' ) ): ?>
					...
					$yoast_breadcrumb = '';
					if ( function_exists( 'yoast_breadcrumb' ) ) {
						$yoast_breadcrumb = yoast_breadcrumb( '<div class="container container-single-page goso-breadcrumb goso-crumb-inside ' . $class_small_width . '">', '</div>', false );
					}

					if ( $yoast_breadcrumb ) {
						echo $yoast_breadcrumb;
					} else { ?>
                        <div class="container container-single-page goso-breadcrumb goso-crumb-inside<?php echo $class_small_width; ?>">
                                <span><a class="crumb"
                                         href="<?php echo esc_url( home_url( '/' ) ); ?>"><?php echo goso_get_setting( 'goso_trans_home' ); ?></a></span><?php goso_fawesome_icon( 'fas fa-angle-right' ); ?>
							<?php
							$page_parent = get_post_ancestors( get_the_ID() );
