...
<article id="post-<?php the_ID(); ?>" class="<?php echo $standard_class; ?>">
	...
	<?php if ( ! get_theme_mod( 'goso_standard_thumbnail' ) ): ?>

		<?php if ( goso_get_post_format( 'link' ) || goso_get_post_format( 'quote' ) ) : ?>
            <div class="standard-post-image standard-post-special<?php if ( goso_get_post_format( 'quote' ) ): ?> goso-special-format-quote<?php endif; ?><?php if ( ! has_post_thumbnail() || get_theme_mod( 'goso_standard_thumbnail' ) ) : echo ' no-thumbnail'; endif; ?>">
				<?php if ( has_post_thumbnail() && ! get_theme_mod( 'goso_standard_thumbnail' ) ) : ?>
					<?php if ( ! get_theme_mod( 'goso_disable_lazyload_layout' ) ) { ?>
                        <a <?php if ( get_theme_mod( 'goso_standard_thumb_crop' ) ): ?> class="goso-image-holder goso-lazy" data-bgset="<?php echo goso_image_srcset( get_the_ID(), 'goso-full-thumb' ); ?>"<?php endif; ?>
                                href="<?php the_permalink() ?>">
							<?php if ( ! get_theme_mod( 'goso_standard_thumb_crop' ) ): ?><img
					...
					<?php } else { ?>
                        <a <?php if ( get_theme_mod( 'goso_standard_thumb_crop' ) ): ?> class="goso-image-holder" style="background-image: url('<?php echo goso_get_featured_image_size( get_the_ID(), 'goso-full-thumb' ); ?>');"<?php endif; ?>
                                href="<?php the_permalink() ?>">
							<?php if ( ! get_theme_mod( 'goso_standard_thumb_crop' ) ): ?><?php the_post_thumbnail( 'goso-full-thumb' ); ?><?php endif; ?>
                        </a>
					<?php } ?>
				<?php endif; ?>
                <div class="standard-content-special">
					<?php if ( goso_get_post_format( 'quote' ) ): ?><a href="<?php the_permalink(); ?>"><?php endif; ?>
                        <div class="format-post-box<?php if ( goso_get_post_format( 'quote' ) ) {
                            ...
                            <span class="post-format-icon"><?php goso_fawesome_icon( 'fa fa-' . ( goso_get_post_format( 'quote' ) ? 'quote-left' : 'link' ) ); ?></span>
                            <p class="dt-special">
								<?php
		...
		<?php elseif ( goso_get_post_format( 'gallery' ) ) : ?>
			...
			<?php if ( $images ) :
				$autoplay = ! get_theme_mod( 'goso_standard_disable_autoplay_gallery' ) ? 'true' : 'false';
				?>
    ...
    <div class="header-standard<?php if ( get_theme_mod( 'goso_standard_meta_overlay' ) ): echo ' standard-overlay-meta'; endif; ?>">
		<?php if ( ! get_theme_mod( 'goso_standard_cat' ) ) : ?>
		...
		<?php $hide_readtime = get_theme_mod( 'goso_standard_readingtime' );
		if ( ! get_theme_mod( 'goso_standard_author' ) || goso_isshow_reading_time( $hide_readtime ) ) : ?>
            <div class="goso-meta-author standard-top-meta author-post byline">
				<?php if ( ! get_theme_mod( 'goso_standard_author' ) ) : ?>
    ...
    <div class="standard-content">
        <div class="standard-main-content entry-content">
            <div class="post-entry standard-post-entry classic-post-entry <?php echo 'blockquote-' . $block_style; ?>">
				<?php if ( get_theme_mod( 'goso_standard_auto_excerpt' ) || isset( $atts['wpblock'] ) ) {
					$default_excerpt         = get_theme_mod( 'goso_post_excerpt_length', 30 );
					$standard_excerpt_length = get_theme_mod( 'goso_standard_excerpt_length', $default_excerpt );
					goso_the_excerpt( $standard_excerpt_length );
					?>
                    <div class="goso-more-link<?php if ( get_theme_mod( 'goso_standard_continue_reading_button' ) ): echo ' goso-more-link-button'; endif; ?>">
		...
		<?php if ( ! get_theme_mod( 'goso_standard_share_box' ) || ! get_theme_mod( 'goso_standard_date' ) || ! get_theme_mod( 'goso_standard_comment' ) || get_theme_mod( 'goso_standard_viewcount' ) ) : ?>
            <div class="goso-post-box-meta<?php if ( get_theme_mod( 'goso_standard_share_box' ) || ( get_theme_mod( 'goso_standard_date' ) && get_theme_mod( 'goso_standard_comment' ) && ! get_theme_mod( 'goso_standard_viewcount' ) ) ): echo ' center-inner'; endif; ?>">
				<?php if ( ! get_theme_mod( 'goso_standard_date' ) || ! get_theme_mod( 'goso_standard_comment' ) || get_theme_mod( 'goso_standard_viewcount' ) ) : ?>
                    <div class="goso-box-meta">
						<?php if ( ! get_theme_mod( 'goso_standard_date' ) ) : ?>
                            <span><?php goso_fawesome_icon( 'far fa-clock' ); ?><?php goso_authow_time_link(); ?></span>
						<?php endif; ?>
						<?php if ( ! get_theme_mod( 'goso_standard_comment' ) ) : ?>
						...
						<?php endif; ?>
						<?php
						if ( get_theme_mod( 'goso_standard_viewcount' ) ) {
							goso_get_post_countview( get_the_ID() );
						}
						?>
                    </div>
				<?php endif; ?>
				<?php if ( ! get_theme_mod( 'goso_standard_share_box' ) ) : ?>
...
if ( isset( $infeed_ads ) && $infeed_ads ) {
	goso_get_markup_infeed_ad(
		array(
			'wrapper'    => 'article',
			'classes'    => 'standard-article standard-pitem post goso-infeed-data',
