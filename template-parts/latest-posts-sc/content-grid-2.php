	...
	<article id="post-<?php the_ID(); ?>" class="item hentry">
				...
				<div class="thumbnail">
					<div class="goso-owl-carousel goso-owl-carousel-slider goso-nav-visible" data-auto="true">
						<?php foreach ( $images as $image ) : ?>
							...
							<?php $the_image = wp_get_attachment_image_src( $image, goso_featured_images_size_vcel( 'normal', $goso_featimg_size, $thumb_size ) ); ?>
							<?php $the_caption = get_post_field( 'post_excerpt', $image ); ?>

							<?php if( ! get_theme_mod( 'goso_disable_lazyload_layout' ) ) { ?>
								<figure class="goso-image-holder goso-lazy" <?php if ($the_caption) : ?> title="<?php echo esc_attr( $the_caption ); ?>"<?php endif; ?> data-bgset="<?php echo esc_url( $the_image[0] ); ?>">
								</figure>
							<?php } else { ?>
								<figure class="goso-image-holder" <?php if ($the_caption) : ?> title="<?php echo esc_attr( $the_caption ); ?>"<?php endif; ?> style="background-image: url('<?php echo esc_url( $the_image[0] ); ?>');">
								</figure>
							<?php } ?>
		...
		<?php elseif ( has_post_thumbnail() ) : ?>
			<div class="thumbnail">
				...
				?>
				<?php if( ! get_theme_mod( 'goso_disable_lazyload_layout' ) ) { ?>
					<a class="goso-image-holder goso-lazy<?php echo goso_class_lightbox_enable(); ?>" data-bgset="<?php echo goso_image_srcset( get_the_ID(), goso_featured_images_size_vcel( 'normal', $goso_featimg_size, $thumb_size ) ); ?>" href="<?php goso_permalink_fix(); ?>" title="<?php echo
					wp_strip_all_tags( get_the_title
					() ); ?>">
					</a>
				<?php } else { ?>
					<a class="goso-image-holder<?php echo goso_class_lightbox_enable(); ?>" style="background-image: url('<?php echo goso_get_featured_image_size( get_the_ID(), goso_featured_images_size_vcel( 'normal', $goso_featimg_size, $thumb_size ) ); ?>');" href="<?php goso_permalink_fix(); ?>"
...
if( isset( $infeed_ads ) && $infeed_ads ){
	goso_get_markup_infeed_ad(
		array(
			'wrapper' => 'li',
			'classes'      => 'grid-style grid-2-style goso-infeed-data goso-infeed-vcele',
