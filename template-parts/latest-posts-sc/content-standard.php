...
<article id="post-<?php the_ID(); ?>" class="<?php echo $standard_class; ?>">
	...
	<?php if ( 'yes' != $atts['standard_thumbnail'] ): ?>
		...
		<?php if ( goso_get_post_format( 'link' ) || goso_get_post_format( 'quote' ) ) : ?>
            <div class="standard-post-image standard-post-special<?php if ( goso_get_post_format( 'quote' ) ): ?> goso-special-format-quote<?php endif; ?><?php if ( ! has_post_thumbnail() || 'yes' == $atts['standard_thumbnail'] ) : echo ' no-thumbnail'; endif; ?>">
				<?php if ( has_post_thumbnail() && 'yes' != $atts['standard_thumbnail'] ) : ?>
					<?php if ( ! get_theme_mod( 'goso_disable_lazyload_layout' ) ) { ?>
                        <a <?php if ( 'yes' == $standard_thumb_crop ): ?> class="goso-image-holder goso-lazy" data-src="<?php echo goso_get_featured_image_size( get_the_ID(), 'goso-full-thumb' ); ?>"<?php endif; ?>
    ...
    <div class="header-standard<?php if ( 'yes' == $atts['standard_meta_overlay'] ): echo ' standard-overlay-meta'; endif; ?>">
		...
		<?php if ( 'yes' != $standard_author || goso_isshow_reading_time( $standard_readtime ) ) : ?>
            <div class="standard-top-meta goso-meta-author author-post byline">
...
if ( isset( $infeed_ads ) && $infeed_ads ) {
	goso_get_markup_infeed_ad(
		array(
			'wrapper'    => 'article',
			'classes'    => 'standard-article standard-pitem post goso-infeed-data goso-infeed-vcele',
