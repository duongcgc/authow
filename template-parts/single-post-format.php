...
<?php if ( goso_get_post_format( 'link' ) || goso_get_post_format( 'quote' ) ) : ?>
	...
	if ( ! has_post_thumbnail() || get_theme_mod( 'goso_standard_thumbnail' ) ) {
...
<?php else : ?>
	...
	<?php if ( has_post_thumbnail() && ! get_theme_mod( 'goso_post_thumb' ) ) : ?>
		...
		if( in_array( $single_style, array( 'style-6','style-5','style-8','style-10' ) ) ){
			$class_stand_style .= ' goso-header-text-white';
		}

		if(  $enable_jarallax ){
			$class_stand_style .= ' goso-jarallax';
		}
		?>
		<div class="<?php echo $class_stand_style; ?>">
			...
			if ( get_the_post_thumbnail_caption() && get_theme_mod( 'goso_post_thumb_caption' ) ) {
				echo '<div class="goso-featured-caption goso-fixed-caption">' . get_the_post_thumbnail_caption() . '</div>';
			}
			?>
		</div>
	<?php endif; ?>
