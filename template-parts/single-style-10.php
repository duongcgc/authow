...
$single_magazine = 'container-single goso-single-style-10 goso-single-smore container-single-fullwidth hentry  goso-header-text-white';
...
$class_container_single = 'container container-single goso-single-style-10 goso-single-smore';
if ( $sidebar_enable ){
		...
		<div class="goso-single-pheader <?php echo ( $single_magazine );?>">
			...
			if ( have_posts() ) :
				while ( have_posts() ) : the_post();
					echo '<div class="goso-post-image-wrapper">';
					echo '<div class="container' . ( 'two-sidebar' == $sidebar_position ? ' two-sidebar' : '' ) . '">';
					if ( ! get_theme_mod( 'goso_move_breadcrumbs' ) ) {
						get_template_part( 'template-parts/single', 'breadcrumb-inner' );
					}
					echo '<div class="post-format-entry-header">';
					echo '<div class="goso-sidebar-content">';
					if ( get_theme_mod( 'goso_move_breadcrumbs' ) ) {
						get_template_part( 'template-parts/single', 'breadcrumb-inner' );
					}
					get_template_part( 'template-parts/single', 'entry-header' );

					if ( get_theme_mod( 'goso_post_adsense_single10' ) ) {
						echo '<div class="goso-google-adsense-single-s10">';
						echo do_shortcode( get_theme_mod( 'goso_post_adsense_single10' ) );
						echo '</div>';
					}

					echo '</div>';
					if ( $show_post_format ) {
									echo '<div class="goso-single-s10-content">';
