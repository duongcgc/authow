	...
	<article id="post-<?php the_ID(); ?>" class="item overlay-layout hentry">
		<div class="goso-overlay-over">
			<div class="thumbnail">
				...
				<a class="goso-image-holder goso-lazy" data-bgset="<?php echo goso_get_featured_image_size( get_the_ID(), 'goso-full-thumb' ); ?>" href="<?php the_permalink(); ?>" title="<?php echo wp_strip_all_tags( get_the_title() ); ?>">
				</a>
				<?php } else { ?>
				<a class="goso-image-holder" style="background-image: url('<?php echo goso_get_featured_image_size( get_the_ID(), 'goso-full-thumb' ); ?>');" href="<?php the_permalink(); ?>" title="<?php echo wp_strip_all_tags( get_the_title() ); ?>">
				</a>
				<?php } ?>
			</div>
			...
			<div class="overlay-header-box">
				...
				<h2 class="goso-entry-title entry-title overlay-title"><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h2>
				<?php goso_authow_meta_schema(); ?>
				<?php if ( ! get_theme_mod( 'goso_grid_author' ) ) : ?>
					<div class="goso-meta-author overlay-author byline"><span class="author-italic author vcard"><?php echo goso_get_setting( 'goso_trans_written_by' ); ?> <a class="url fn n" href="<?php echo get_author_posts_url( get_the_author_meta( 'ID' ) ); ?>"><?php the_author(); ?></a></span></div>
				<?php endif; ?>
			</div>
		</div>
		<?php $hide_readtime = get_theme_mod( 'goso_grid_readingtime' ); ?>
		<?php if ( ! get_theme_mod( 'goso_grid_date' ) || ! get_theme_mod( 'goso_grid_comment' ) || ! get_theme_mod( 'goso_grid_share_box' ) || get_theme_mod( 'goso_grid_countviews' ) || goso_isshow_reading_time( $hide_readtime ) ) : ?>
			<div class="goso-post-box-meta grid-post-box-meta overlay-post-box-meta">
				...
				<?php if ( ! get_theme_mod( 'goso_grid_comment' ) ) : ?>
					<div class="overlay-share overlay-style-comment"><a href="<?php comments_link(); ?> "><?php goso_fawesome_icon('far fa-comment'); ?><?php comments_number( '0 ' . goso_get_setting( 'goso_trans_comment' ), '1 '. goso_get_setting( 'goso_trans_comment' ), '% ' . goso_get_setting( 'goso_trans_comments' ) ); ?></a></div>
				<?php endif; ?>
...
if( isset( $infeed_ads ) && $infeed_ads ){
	goso_get_markup_infeed_ad(
		array(
			'wrapper' => 'section',
			'classes'      => 'grid-style grid-overlay goso-infeed-data',
